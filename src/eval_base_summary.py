...
def main():
    ...
    parser.add_argument('--model', type=str, 
                      choices=['bart', 'bart-large', 'pegasus', 'gpt', 'qwen', 'all'], 
                      default='all', help='Model to evaluate')
    args = parser.parse_args()
    
    models = []
    if args.model == 'all' or args.model == 'bart':
        models.append(BaselineModel("facebook/bart-base", "summary", hf_token))
    if args.model == 'all' or args.model == 'bart-large':
        models.append(BaselineModel("facebook/bart-large", "summary", hf_token))
    ...
    if args.model == 'all' or args.model == 'qwen':
        models.append(BaselineModel("Qwen/Qwen2.5-1.5B-Instruct", "summary", hf_token))
    ...
    for model in models:
        ...
        with open(f'src\summary_eval_results\summary_evaluation_{model.model_name.split("/")[-1]}.json', 'w') as f:
    ...
    if len(models) > 1:
        ...
        with open('src\summary_eval_results\summary_evaluation_results.json', 'w') as f:
